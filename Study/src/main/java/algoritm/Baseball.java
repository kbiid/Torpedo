...
public class Baseball {
	...
	private static int solution(int[][] baseball) {
		...
		for (int i = baseball_minValue; i <= baseball_maxValue; i++) {// 완전 탐색 알고리즘을 위한 반복문
			compareValue_array = setArray(compareValue_array, i);

			if (compareValue_array[0] == 0 || compareValue_array[1] == 0 || compareValue_array[2] == 0) {// 0인지 검사
				continue;
			}
			if (compareValue_array[0] == compareValue_array[1] || compareValue_array[1] == compareValue_array[2]
					|| compareValue_array[2] == compareValue_array[0]) {// 중복된 값이 있는지 검사
				continue;
			}

			for (int j = 0; j < baseball.length; j++) {// 정답과 입력된 값들을 비교하여 가능한 정답의 갯수를 측정
				strike_count = 0;
				ball_count = 0;
